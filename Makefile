build-all:
	cd registry; cargo concordium build -e --out registry.wasm.v1; cd ../staking-bank; cargo concordium build -e --out staking_bank.wasm.v1 -- --features development; cargo concordium build -e -- --features sandbox; cargo concordium build -e -- --features production; cd ../umbrella-feeds; cargo concordium build -e --out umbrella_feeds.wasm.v1; cd ../umbrella-feeds-reader; cargo concordium build -e --out umbrella_feeds_reader.wasm.v1; cd ../dummy-contract; cargo concordium build -e --out dummy-contract.wasm.v1;

test-all:
	cd registry; cargo concordium test; cd ../staking-bank; cargo concordium test -- --features development; cd ../umbrella-feeds; cargo concordium test; cd ../umbrella-feeds-reader; cargo concordium test;
